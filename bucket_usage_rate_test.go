...
func (rateLimitSuite) TestRate1(c *gc.C) {
	...
	log.addLog(now, 5*time.Second, 10)
...
func (rateLimitSuite) TestRate2(c *gc.C) {
	...
	c.Assert(log.getRate(now), gc.Equals, int64(150/5))
	now = now.Add(1 * time.Second)
	c.Assert(log.getRate(now), gc.Equals, int64(140/5))
	now = now.Add(1 * time.Second)
	c.Assert(log.getRate(now), gc.Equals, int64(120/5))
	now = now.Add(1 * time.Second)
	c.Assert(log.getRate(now), gc.Equals, int64(90/5))
	now = now.Add(1 * time.Second)
	c.Assert(log.getRate(now), gc.Equals, int64(50/5))
	now = now.Add(1 * time.Second)
	c.Assert(log.getRate(now), gc.Equals, int64(0/5))
